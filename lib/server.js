...
const getAvailableModules = coroutine(function*() {
  try {
    const projectPackage = JSON.parse(
      yield fs.readFile(process.cwd() + '/package.json')
    )
    return Object.keys(projectPackage.dependencies || {})
  } catch (err) {
    console.log(err.message)
    console.log(
      chalk.red(
        '\n' +
          'Serving without a package.json for the project.\n' +
          'You can not load modules from /libs/'
      )
    )
    return []
  }
...
getAvailableModules().then(availableModules => {
...
module.exports = coroutine(function*(req, res, flags, current, ignoredFiles) {
  ...
  const availableModules = yield getAvailableModules()

  // For loading of external libraries, we
  // use the /libs/ prefix.
  //
  // Note: Only packages with a module field are supported.
  //
  if (availableModules && pathname.startsWith('/libs/')) {
    res.setHeader('Content-type', 'application/javascript')
    const split = pathname.substring('/libs/'.length).split('/')
    const libraryName = split[0]
    if (availableModules.includes(libraryName)) {
      const libraryPath = resolvePkg(libraryName)
      const libraryPackage = require(libraryPath + '/package.json')

      if (split.length > 1) {
        const filePath = path.resolve(libraryPath, split.slice(1).join('/'))
        return res.end(yield fs.readFile(filePath))
      }

      if (libraryPackage.module) {
        const modulePath = path.resolve(libraryPath, libraryPackage.module)
        console.log(
          chalk.green(`Loading "${libraryName}" from "${modulePath}"`)
        )
        const fileContent = yield fs.readFile(modulePath)
        return res.end(fileContent)
      }

      const rawContent = yield fs.readFile(
        path.resolve(libraryPath, libraryPackage.main)
      )
      const fileContent =
        'var module = {}; var exports = module.exports = {};' +
        rawContent +
        '; export default module.exports'

      console.log('Using compat version of: ' + libraryName)

      return res.end(fileContent)
    }

    const msg = `The library "${libraryName}" was not found`
    console.log(chalk.red(msg))
    return micro.send(res, 404, msg)
  }
